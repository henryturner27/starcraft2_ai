...
"""Scripted agents for solving the various mini-games."""
                ...
                if not self.rally_set:
                    if _RALLY_WORKERS not in obs.observation["available_actions"]:
                        ...
                        return actions.FunctionCall(_SELECT_POINT,
                                                    [_NOT_QUEUED, [command_center[0], command_center[1]]])
                    else:
                        self.rally_set = True
                        return actions.FunctionCall(_RALLY_WORKERS, [_NOT_QUEUED, self.target])
                else:
                    try:
                        if (obs.observation["player"][3] < obs.observation["player"][4]) & (
                                    obs.observation["player"][1] >= 50) & (obs.observation["player"][6] < 30) & (
                                    obs.observation["build_queue"][0][6] >= 90) & (
                            len(obs.observation["build_queue"]) < 2):
    ...
    def step(self, obs):
        ...
        if self.initial_worker_selection:
            ...
            if (obs.observation["player"][3] < obs.observation["player"][4]) & (obs.observation["player"][1] >= 50) & (
                        obs.observation["player"][6] < 30) & (_TRAIN_SCV in obs.observation["available_actions"]):
                if (len(obs.observation["build_queue"]) == 1) & (
                                (obs.observation["player"][6]) + len(obs.observation["build_queue"]) < 30):
...
class BuildMarines(BaseAgent):
    ...
    def step(self, obs):
        ...
        if self.initial_worker_selection:
            ...
            # train SCVs
            if (obs.observation["player"][3] < obs.observation["player"][4]) & (obs.observation["player"][1] >= 50) & (
                        obs.observation["player"][6] < 16) & (_TRAIN_SCV in obs.observation["available_actions"]):
                if (len(obs.observation["build_queue"]) == 1) & (
                                (obs.observation["player"][6]) + len(obs.observation["build_queue"]) < 16):
                    if (obs.observation["build_queue"][0][6] >= 90):
                        print('train scv 422')
                        return actions.FunctionCall(_TRAIN_SCV, [_NOT_QUEUED])
                elif (len(obs.observation["build_queue"]) == 0) & (
